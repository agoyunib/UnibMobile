...
$("#cekDataRemun").click(function () {
    ...
    if (kodeRubrik) {
        ...
        $.ajax({
            type: "get",
            url: "skp/kewajibankhususprosessor/getRubrik",
            data: {
...
$("#tambahData").click(function () {
    ...
    $("#formEdit")
        .attr("action", "skp/kewajibankhususprosessor/insertKewajiban")
        .trigger("reset");
});

$("#table").on("click", "#editData", function () {
    let idSkpKewajiban = $(this).data("idskpkewajiban");
    $("#modalTitle").text("Edit Data");
    $("#formEdit").attr(
        "action",
        "skp/kewajibankhususprosessor/updateKewajiban"
    );
    $("#modalEdit").modal("show");
    $.ajax({
        url: "skp/kewajibankhususprosessor/editKewajiban",
        type: "get",
        data: {
            id: idSkpKewajiban,
        },
...
$("#table").on("click", "#deleteData", function () {
    ...
    swal.fire({
        ...
        if (confirm.value) {
            $.ajax({
                url:
                    "skp/kewajibankhususprosessor/deleteKewajiban/" +
                    idSkpKewajiban,
                type: "get",
                success: function (result) {
                    if (result > 0) {
                        swal.fire(
                            "Terhapus !",
                            "Data Skp Kewajiban Khusus Profesor berhasil dihapus !",
                            "success"
                        );
                    }
                    location.reload();
